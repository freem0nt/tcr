...
$my_email = "4787773201@txt.att.net";
...
if (isset($_REQUEST['smsname']) && !empty($_REQUEST['smsname'])) {

    if (preg_match("/[^a-z' -]/i", stripslashes($_REQUEST['smsname']))) {
...
if (isset($_REQUEST['smsnumber']) && !empty($_REQUEST['smsnumber'])) {

    if (preg_match('/[^0-9]/i', stripslashes($_REQUEST['smsnumber']))) {
        $errors[] = "You have entered an invalid character in the number field";
    }
...
$smsmessage = build_message($_REQUEST);
...
//$smssubject = 'Text from TCR';

//$smsemail = $email;

$headers = 'From: ' . $smsnumber . '\r\n' .
    'Reply-To: ' . $smsnumber . '\r\n';

mail($my_email, $smssubject, $smsmessage, $headers);
...
<div id="content">
    ...
    <p><a href="<?php print $continue; ?>">Click here</a> to go back to my home page.</p>
    <div id="wrapper">
        <dl>
